...
DATAPATH = '/data/cj/dataset/aircraft'
FILENAME_LENGTH = 7
...
class AircraftDataset(Dataset):
    ...
    def __init__(self, phase='train', data_path=DATAPATH, resize=500):
        ...
        variants_dict = {}
        with open(os.path.join(self.data_path, 'variants.txt'), 'r') as f:
            for idx, line in enumerate(f.readlines()):
                variants_dict[line.strip()] = idx
        self.num_classes = len(variants_dict)

        if phase == 'train':
            list_path = os.path.join(self.data_path, 'images_variant_trainval.txt')
        else:
            list_path = os.path.join(self.data_path, 'images_variant_test.txt')
    ...
    def __getitem__(self, item):
        # image
        image = Image.open(os.path.join(self.data_path, 'images', '%s.jpg' % self.images[item])).convert('RGB')  # (C, H, W)
